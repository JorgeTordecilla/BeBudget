...
describe("analytics api wrappers", () => {
  it("uses vendor accept header and credentials include for analytics requests", async () => {
    const fetchMock = vi
      ...
      .mockResolvedValueOnce(
        new Response(JSON.stringify({ impulse_count: 0, intentional_count: 0, untagged_count: 0, top_impulse_categories: [] }), {
          status: 200,
          headers: { "content-type": "application/json" }
        })
      );
    ...
    await getImpulseSummary(client, { from: "2026-02-01", to: "2026-02-28" });
  ...
  it("propagates ProblemDetails for 400/401/406 failures", async () => {
    ...
    await expect(getImpulseSummary(client, { from: "2026-03-01", to: "2026-03-31" })).rejects.toMatchObject({ status: 400 });
  });
