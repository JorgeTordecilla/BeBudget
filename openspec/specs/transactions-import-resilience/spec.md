...
The backend MUST accept import requests without tying up API request threads for full import execution.
...
#### Scenario: Client can query job status
- **WHEN** client requests import job status
...
Repeated import submissions with the same idempotency key MUST not duplicate side effects.

#### Scenario: Same idempotency key reuses existing job outcome
- **WHEN** client retries `POST /transactions/import/jobs` with the same `Idempotency-Key`
...
Operations MUST expose enough signals to validate resilience behavior.

#### Scenario: Queue and rejection metrics are emitted
- **WHEN** imports are processed or throttled
- **THEN** system emits metrics/logs for queue depth, active workers, accept/reject counts, and job outcomes.

#### Scenario: Resilience tests cover burst traffic behavior
- **WHEN** automated validation runs
- **THEN** tests verify that burst submission (representative high concurrency) triggers controlled queueing/throttling instead of service unavailability.
